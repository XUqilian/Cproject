...
int seven_two(void)
{
    ...
    //浮点数是不能用 unsigned 来规范的.unsigned 的意思就是把内存中的数据第一位也用来表示数据,而不用于表示符号位.
    //而浮点数规定内存中数据的第一位必须是符号位.因此两者之间是互相矛盾的,这也就是为什么浮点数不会有 unsigned 类型.
    double golf[10] = {0};
...
int seven_six(void)
{
    ...
    // func1(ary,size)
    int i = size;
    while (i-- > 0 && cin >> ary[i])    //未重载的'&&'运算符的左操作数赋值之后 (即"&&"处) 是顺序点
        ;
    if (cin.fail()) //直接把 cin 当条件也可以
    {
        cout << "enter error!" << endl;
        cin.clear();
        while (cin.get() != '\n')
...
int seven_eight(void)
{
    ...
    for (back; ++back != (size - i + 1);)
    {
        ...
        tp[back].num = ary[back];
        // tp[back].name = chs[back];  //name 是一个数组,数组是不能被赋给数组的只能一个一个 element 赋值
        for (int j = 0; ary[j]; j++)
...
double (*seven_ten_s[4])(double, double); //创建 4 个同返回类型同参数的函数 分管加减乘除
int ten(void)
{
