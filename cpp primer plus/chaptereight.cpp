...
{
    for (int i = 0; str[i] ; i++) // Ansic 码 0:32 \0:48  0:48
    {
        str[i] = toupper(str[i]);
    }
...
char *maxn<char *>(char *ary[], int i)
{
    ...
    while (i--)
    {
//       if (strcmp(ary[i], temp) > 0)   //这个不行呀,这是比较字符串,并不比较长度呀
        if( strlen(ary[i]) > strlen(temp) )
            strcpy(temp, ary[i]);
    }
