//编写程序时能使用c语法便用c语法（快速响应），但如果复杂就转用c++
//能不用using编译指令（using namespace ...)就不用，保证后期兼容性与拓展性，多使用using声明(using std::cin)
//或者直接引入 std::cin
...
void sumstr(const std::string &str) //使用了域符号
{
    static int sum = 0;
    sum += str.length();
    printf("%d\t%d\t%d", str.length(), sizeof(str), sum); //附带看一下sizeof的大小
}
...
inline void setchaff(chaff &stc, const char *str, int size) //结构简单且调用频繁，使用inline将其定为内联函数
{
