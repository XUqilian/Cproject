...
int six_two(void)
{
    ...
    //     ary[i]=temp;
    //     cin>>temp;
    // }   /有问题，cin获取不同类型数据时会出现什么现象，用isdigit不适合

    for (; cin >> temp && i < Cnt; i++) // cin遇到不符合类型的输入时会返回false
    {
        ary[i] = temp;
    } //循环后i和输入数量一致，因为i在每一次循环结束后都会增加
...
int six_four(void)
{
    ...
    int num_of_ary; // let让 user dicesion决定 number of people.
...
int six_five(void)
{
    ...
    while (cin >> revenue && revenue > 0)
    {
        ...
        if (revenue <= 5000)
            rate = 0;
        else if (revenue <= 1500 && revenue > 5000)
            rate = (revenue - 5000) * 0.1;
        else if (revenue <= 3500 && revenue > 1500)
            rate = (1000 * 0.1) + (revenue - 1500) * 0.15;
        else
            rate = (1000 * 0.1) + (2000 * 0.15) + (revenue * 0.2);
    }
...
int six_eight(void)
{
    ...
    cout << "enter the file path、 filename and suffix:"; // suffix后缀
...
int six_ten(void)
{
    ...
    for (int i = 0; i < people; i++)
        delete dons[i]; //释放堆上的存储空间
    // delete[] dons;   错误的方法，因为dons并不是动态分配的，它的元素才是
